...
from data import ExecutionTier, SchedulingState, core_execution_times, cloud_execution_times, edge_execution_times, \
...
class PSOParams:
    ...
    alpha: float = 1.0  # weight for completion time in fitness
    beta: float = 1.0  # weight for energy consumption in fitness
    penalty: float = 1e6


class Particle:
    def __init__(self, dim, resource_range):
        # Position: continuous vector of length dim; each dimension in [0, resource_range-1]
        self.position = np.random.uniform(0, resource_range, dim)
        # Velocity: initialize with small random values
...
def apply_migration_configuration(tasks, migration_vector):
    """
    Given a migration_vector (list of integers of length len(tasks)),
    update each task's execution assignment based on the resource index.
    Uses the function get_execution_unit_from_index to map the index.
    Returns a deep copy of the tasks with updated assignments.
    """
    tasks_copy = deepcopy(tasks)
    for idx, task in enumerate(tasks_copy):
        # Round the continuous value to nearest integer resource index.
        resource_idx = int(round(migration_vector[idx]))
        # Ensure resource index is within valid bounds:
        # Let total_units = num_device_cores + (num_edge_nodes * edge_cores_per_node) + 1 (for cloud)
        # We assume get_execution_unit_from_index() will handle out-of-bound values if needed.
        exec_unit = get_execution_unit_from_index(resource_idx)
        # Update task assignment accordingly:
        if exec_unit.tier == ExecutionTier.DEVICE:
            task.execution_tier = ExecutionTier.DEVICE
            task.device_core = exec_unit.location[0]
            task.edge_assignment = None
        elif exec_unit.tier == ExecutionTier.EDGE:
            task.execution_tier = ExecutionTier.EDGE
            task.device_core = -1
            # Convert 0-based to 1-based for assignment:
            task.edge_assignment = EdgeAssignment(edge_id=exec_unit.location[0] + 1, core_id=exec_unit.location[1] + 1)
        else:
            task.execution_tier = ExecutionTier.CLOUD
            task.device_core = -1
            task.edge_assignment = None
...
def pso_fitness(migration_vector, tasks, sequence_manager, T_final, system_params, pso_params):
    """
    Evaluate a migration configuration represented by migration_vector.
    The fitness is computed as a weighted sum of the normalized completion time
    and normalized energy consumption, plus a dynamic quadratic penalty if the
    completion time exceeds T_final.

    Normalization factors are derived from the initial schedule's metrics:
      normalized_completion_time = comp_time / initial_completion_time
      normalized_energy = energy / initial_energy
    """
    # Apply the migration configuration (using our helper function)
    tasks_candidate = apply_migration_configuration(tasks, migration_vector)

    # Run kernel rescheduling to update timings based on the new assignments
    tasks_candidate = kernel_algorithm_3tier(tasks_candidate, sequence_manager,
                                             system_params.upload_rates, system_params.download_rates)

    # Compute overall completion time and energy consumption for this candidate schedule.
    comp_time = total_time_3tier(tasks_candidate)
    energy = total_energy_3tier_with_rf(tasks_candidate,
                                        device_power_profiles=system_params.power_models['device'],
                                        rf_power=system_params.power_models['rf'],
                                        upload_rates=system_params.upload_rates,
                                        default_signal_strength=system_params.default_signal_strength)

    # Retrieve the initial schedule metrics (set before calling PSO)
    init_T = system_params.initial_completion_time
    init_E = system_params.initial_energy

    # Normalize the metrics: this makes the fitness function scale‐independent.
    norm_time = comp_time / init_T if init_T > 0 else comp_time
    norm_energy = energy / init_E if init_E > 0 else energy

    # Compute a dynamic penalty: if completion time exceeds T_final, add a quadratic penalty.
    penalty = 0.0
    if comp_time > T_final:
        # For example, the penalty factor can be scaled by the maximum of the normalized values.
        dynamic_factor = max(norm_time, norm_energy)
        penalty = pso_params.penalty * ((comp_time / T_final) - 1) ** 2 * dynamic_factor

    # Compute overall fitness: lower fitness is better.
    fitness = pso_params.alpha * norm_time + pso_params.beta * norm_energy + penalty
    return fitness
...
class SystemParams:
    ...
    download_rates: dict
    power_models: dict
    default_signal_strength: float = 70.0
    initial_completion_time: float = 0.0  # New attribute for dynamic normalization
    initial_energy: float = 0.0


def pso_migration_optimization(tasks, sequence_manager, T_final, system_params, pso_params):
    """
    PSO-based migration decisioning.

    Args:
        tasks: List of Task objects (current schedule assignments).
        sequence_manager: Current SequenceManager.
        T_final: Completion time constraint (e.g., target or maximum allowed time).
        system_params: SystemParams object containing upload_rates, download_rates, power_models, etc.
        pso_params: PSOParams object.

    Returns:
        best_migration: Best migration vector (discrete, list of integers).
        best_fitness: Fitness value of the best migration.
    """
    num_tasks = len(tasks)
    # Determine total number of resources (R) from the sequence manager.
    # Assume sequence_manager.total_units is available; otherwise, compute from configuration.
    total_resources = sequence_manager.total_units if hasattr(sequence_manager, 'total_units') else \
        (
                sequence_manager.num_device_cores + sequence_manager.num_edge_nodes * sequence_manager.num_edge_cores_per_node + 1)

    # Initialize population of particles.
    population = [Particle(num_tasks, total_resources) for _ in range(pso_params.population_size)]

    # Evaluate initial fitness
    for particle in population:
        particle.fitness = pso_fitness(particle.position, tasks, sequence_manager, T_final, system_params, pso_params)
    ...
    # Main PSO loop
    for iter in range(pso_params.max_iterations):
        for particle in population:
            # Update velocity
            r1 = np.random.rand(num_tasks)
            r2 = np.random.rand(num_tasks)
            cognitive = pso_params.c1 * r1 * (particle.pbest_position - particle.position)
            social = pso_params.c2 * r2 * (gbest - particle.position)
            particle.velocity = pso_params.w * particle.velocity + cognitive + social

            # Update position
            particle.position = particle.position + particle.velocity
            # Clip positions to [0, total_resources - 1]
            particle.position = np.clip(particle.position, 0, total_resources - 1)

            # Evaluate new fitness
            particle.fitness = pso_fitness(particle.position, tasks, sequence_manager, T_final, system_params,
                                           pso_params)
        ...
        current_gbest_particle = min(population, key=lambda p: p.fitness)
        if current_gbest_particle.fitness < gbest_fitness:
            gbest_fitness = current_gbest_particle.fitness
            gbest = current_gbest_particle.position.copy()

        logger.info(f"PSO Iteration {iter + 1}/{pso_params.max_iterations}: Best Fitness = {gbest_fitness:.4f}")

    # Convert global best continuous vector to discrete migration configuration
    best_migration = [int(round(x)) for x in gbest]
    return best_migration, gbest_fitness


def optimize_task_scheduling_with_pso(tasks, sequence_manager, T_final,
                                      system_params, pso_params):
    """
    Replace the simple iterative migration optimization with a PSO-based migration decisioning module.

    Args:
        tasks: List of Task objects.
        sequence_manager: Current SequenceManager.
        T_final: Completion time constraint.
        system_params: SystemParams object.
        pso_params: PSOParams object.

    Returns:
        optimized_tasks: List of Task objects after applying PSO-based migration decisions and kernel rescheduling.
    """
    # Run PSO-based migration decisioning to get best migration configuration.
    best_migration, best_fitness = pso_migration_optimization(tasks, sequence_manager, T_final, system_params,
                                                              pso_params)
    logger.info(f"PSO-based migration selected configuration: {best_migration} with fitness {best_fitness:.4f}")

    # Apply the best migration configuration to update task assignments.
    tasks_updated = apply_migration_configuration(tasks, best_migration)

    # Re-run kernel rescheduling to update start/finish times.
    optimized_tasks = kernel_algorithm_3tier(tasks_updated, sequence_manager,
                                             system_params.upload_rates, system_params.download_rates)
...
if __name__ == "__main__":
    ...
    predefined_tasks = [task1, task2, task3, task4, task5, task6, task7, task8, task9,
    ...
    for t in tasks:
        ...
        print(f"Task {t.id}: Type={task_type}, Tier={tier_str}, C={t.complexity:.2f}, D={t.data_intensity:.2f}")
        print(f"  Predecessors: {preds} | Successors: {succs}")
        print("  DataSizes:", t.data_sizes)
        print()

    # 5) Primary assignment: assign each task to the best single-task location
    ...
    # 10) Calculate total time and energy for the initial schedule
    ...
    # ========================================================
    # STEP TWO: ENERGY OPTIMIZATION PHASE USING PSO-BASED MIGRATION
    # ========================================================
    if is_valid:  # Proceed only if the initial schedule is valid
        print("\n=== STARTING ENERGY OPTIMIZATION PHASE (PSO-based Migration) ===")
        print(f"Initial completion time: {T_total:.2f}")
        print(f"Initial energy consumption: {E_total:.2f} Joules")

        # Create a new SequenceManager from the current tasks
        ...
        # Set a time constraint: allow up to 20% increase in completion time
        T_max = T_total * 1.2

        # Define system parameters for PSO, including the initial schedule metrics
        system_params = SystemParams(
            upload_rates=upload_rates,
            download_rates=download_rates,
            power_models={'device': device_power_profiles, 'rf': wireless_rf_power_profiles},
            default_signal_strength=70.0,
            initial_completion_time=T_total,
            initial_energy=E_total
        )

        # Define PSO parameters (you may adjust these further)
        ...
        # Run the PSO-based migration optimization (this uses our dynamic fitness function)
        print("\nRunning PSO-based task migration optimization...\n")
        optimized_tasks = optimize_task_scheduling_with_pso(
            tasks=tasks,
            sequence_manager=sequence_manager,
            T_final=T_max,
        ...
        # Calculate new metrics for the optimized schedule
